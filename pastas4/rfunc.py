...
__all__ = ["Gamma", "Exponential", "Hantush", "Polder", "FourParam",
           "DoubleExponential", "One", "Edelman", "HantushWellModel", "DoubleGamma"]
...
class Gamma_rch(RfuncBase):
    ...
    def get_init_parameters(self, name):
        parameters = DataFrame(
            columns=['initial', 'pmin', 'pmax', 'vary', 'name'])
        if self.up:
            parameters.loc[name + '_A_rch'] = (1 / self.meanstress, 1e-5,
                                           100 / self.meanstress, True, name)
        elif self.up is False:
            parameters.loc[name + '_A_rch'] = (-1 / self.meanstress,
                                           -100 / self.meanstress,
                                           -1e-5, True, name)
        else:
            parameters.loc[name + '_A_rch'] = (1 / self.meanstress,
        ...
        # if n is too small, the length of response function is close to zero
        parameters.loc[name + '_n_rch'] = (1, 0.1, 100, True, name)
        parameters.loc[name + '_a_rch'] = (10, 0.01, 1e4, True, name)
        return parameters
...
class Exponential_rch(RfuncBase):
    """Exponential response function with 2 parameters: A_rch and a_rch.
    This response function should be used if a two-component recharge 
    model is used and the second response function is also an Exponential.
    ...
    def get_init_parameters(self, name):
        ...
        parameters.loc[name + '_a_rch'] = (10, 0.01, 1000, True, name)
        return parameters
...
class FourParam_rch(RfuncBase):
    """Four Parameter response function with 4 parameters A_rch, a_rch, b_rch,
    and n_rch.
    This response function should be used if a two-component recharge 
    model is used and the second response function is also a FourParam
    response function.
    ...
    def get_init_parameters(self, name):
        parameters = DataFrame(
            columns=['initial', 'pmin', 'pmax', 'vary', 'name'])
        if self.up:
            parameters.loc[name + '_A_rch'] = (1 / self.meanstress, 0,
                                           100 / self.meanstress, True, name)
        elif self.up is False:
            parameters.loc[name + '_A_rch'] = (-1 / self.meanstress,
                                           -100 / self.meanstress, 0, True,
        ...
        parameters.loc[name + '_n_rch'] = (1, -10, 10, True, name)
        parameters.loc[name + '_a_rch'] = (10, 0.01, 5000, True, name)
        parameters.loc[name + '_b_rch'] = (10, 1e-6, 25, True, name)
        return parameters
...
class DoubleExponential(RfuncBase):
    """Gamma response function with 4 parameters A, alpha, a1 and a2.
    ...
    def step(self, p, dt=1, cutoff=None, maxtmax=None):
        ...
        s = p[0] * (1 - ((1 - p[1]) * np.exp(-t / p[2]) +
                         p[1] * np.exp(-t / p[3])))
        # s = p[0] * (p[2] * (p[1] - 1) * (np.exp(-t / p[2]) - 1) + p[3] * p[1] * (1 - np.exp(-t / p[3])))
        # s = A * (a1 * (alpha - 1) * (e^(-t/a_1) - 1) + a2 * alpha * (1 - e^(-t/a2)))
        return s


class DoubleExponential2(RfuncBase):
    """Gamma response function with 5 parameters A1, A2, a1, a2, n1, n2, alpha
    ...
    .. math:: \\theta(t) = A1 (1 - \\alpha) e^{-(t-n1)/a_1} + A2 \\alpha e^{-(t-n2)/a_2}

    """
    _name = "DoubleExponential2"
    ...
    def get_init_parameters(self, name):
        parameters = DataFrame(
            columns=['initial', 'pmin', 'pmax', 'vary', 'name'])
        if self.up:
            parameters.loc[name + '_A1'] = (1 / self.meanstress, 0,
                                           100 / self.meanstress, True, name)
            parameters.loc[name + '_A2'] = (1 / self.meanstress, 0,
                                           100 / self.meanstress, True, name)
        elif self.up is False:
            parameters.loc[name + '_A1'] = (-1 / self.meanstress,
                                           -100 / self.meanstress, 0, True,
                                           name)
            parameters.loc[name + '_A2'] = (-1 / self.meanstress,
                                           -100 / self.meanstress, 0, True,
                                           name)
        else:
            parameters.loc[name + '_A1'] = (1 / self.meanstress,
        ...
        parameters.loc[name + '_alpha'] = (0.5, 0.01, 0.99, True, name) # 2
        parameters.loc[name + '_a1'] = (10, 0.01, 5000, True, name) # 3
        parameters.loc[name + '_a2'] = (10, 0.01, 5000, True, name) # 4
        parameters.loc[name + '_n1'] = (0, -5000, 5000, True, name) # 5
        parameters.loc[name + '_n2'] = (0, -5000, 5000, True, name) # 6
        return parameters
    ...
    def step(self, p, dt=1, cutoff=None, maxtmax=None):
        t = self.get_t(p, dt, cutoff, maxtmax)
        int1 = p[3] * (- p[0]) * (p[2] - 1) * (np.exp(t / p[3]) - 1) * np.exp((p[5] - t) / p[3])
        int2 = p[4] * p[1] * p[2] * (np.exp(t / p[4]) - 1) * np.exp((p[6] - t) / p[4])
        s = int1 + int2
        return s


class ModifiedWeibull(RfuncBase):
    """Modified Weibull step function with 7 parameters A1, A2, a1, a2, b1, b2, alpha
    ...
    .. math:: derivative of step function

    """
    _name = "ModifiedWeibull"
    ...
    def get_init_parameters(self, name):
        ...
        parameters.loc[name + '_a1'] = (1, 1e-5, 50, True, name) # 2
        parameters.loc[name + '_a2'] = (1, 1e-5, 50, True, name) # 3
        parameters.loc[name + '_b1'] = (1, 1e-5, 5000, True, name) # 4
        parameters.loc[name + '_b2'] = (1, 1e-5, 5000, True, name) # 5
        parameters.loc[name + '_alpha'] = (0.5, 0.01, 0.99, True, name) # 6
        return parameters
    ...
    def step(self, p, dt=1, cutoff=None, maxtmax=None):
        t = self.get_t(p, dt, cutoff, maxtmax)
        term1 = p[0] * (1 - np.exp(- (p[2] * t) ** p[4]))
        term2 = p[1] * (1 - np.exp(- (p[3] * t) ** p[5]))
        s = p[6] * term1 + (1 - p[6]) * term2
        return s


class Dagum(RfuncBase):
    """Dagum distribution step function with 9 parameters A1, A2, a1, a2, b1, b2, p1, p2, alpha
    ...
    def get_init_parameters(self, name):
        parameters = DataFrame(
            columns=['initial', 'pmin', 'pmax', 'vary', 'name'])
        if self.up:
            parameters.loc[name + '_A1'] = (10 / self.meanstress, 0,
                                            1000, True, name)
            parameters.loc[name + '_A2'] = (10 / self.meanstress, 0,
                                            1000, True, name)
        elif self.up is False:
            parameters.loc[name + '_A1'] = (-10 / self.meanstress,
                                            -1000, 0, True,
                                            name)
            parameters.loc[name + '_A2'] = (-10 / self.meanstress,
                                            -1000, 0, True,
                                            name)
        else:
            parameters.loc[name + '_A1'] = (10 / self.meanstress,
        ...
        parameters.loc[name + '_a1'] = (1.0, 1e-2, 5000, True, name) # 2
        parameters.loc[name + '_a2'] = (1.0, 1e-2, 5000, True, name) # 3
        parameters.loc[name + '_b1'] = (1.5, 1e-2, 5000, True, name) # 4
        parameters.loc[name + '_b2'] = (1.5, 1e-2, 5000, True, name) # 5
        parameters.loc[name + '_p1'] = (10.0, 1e-2, 5000, True, name) # 6
        parameters.loc[name + '_p2'] = (10.0, 1e-2, 5000, True, name) # 7
        parameters.loc[name + '_alpha'] = (0.5, 0.01, 0.99, True, name) # 8
        return parameters

    def get_tmax(self, p, cutoff=None):
        if cutoff is None:
            cutoff = self.cutoff
        # term1 = 0.5 * p[2] * ((cutoff ** (- 1 / p[6])) - 1) ** (- 1 / p[4])
        # term2 = 0.5 * p[3] * ((cutoff ** (- 1 / p[7])) - 1) ** (- 1 / p[5])
        # return term1 + term2

        # analytical solution not yet found correctly

        return 10000.0

    def gain(self, p):
        return p[8] * p[0] + (1 - p[8]) * p[1]

    def step(self, p, dt=1, cutoff=None, maxtmax=None):
        t = self.get_t(p, dt, cutoff, maxtmax)
        term1 = p[0] * ((1 + ((t / p[2]) ** -p[4])) ** -p[6])
        term2 = p[1] * ((1 + ((t / p[3]) ** -p[5])) ** -p[7])
        s = p[8] * term1 + (1 - p[8]) * term2
        return s


class SingleDagum(RfuncBase):
    """Dagum distribution step function with 4 parameters A1, a1, b1, p1
    ...
    def __init__(self, up=True, meanstress=1, cutoff=0.9):
        RfuncBase.__init__(self, up, meanstress, cutoff)
        self.nparam = 4
    ...
    def get_init_parameters(self, name):
        ...
        parameters.loc[name + '_a'] = (1, 1e-5, 5000, True, name) # 1
        parameters.loc[name + '_b'] = (4, 1e-5, 5000, True, name) # 2
        parameters.loc[name + '_p'] = (1, 1e-5, 5000, True, name) # 3
        return parameters
    ...
    def get_tmax(self, p, cutoff=None):
        ...
        tmax_calc = p[1] * ((cutoff ** (- 1 / p[3])) - 1) ** (- 1 / p[2])
        ...
        # analytical solution causes overflow for some parameter estimates
        # function is not cut off but rather used to the point where
        # t = 5000.0
    ...
    def step(self, p, dt=1, cutoff=None, maxtmax=None):
        t = self.get_t(p, dt, cutoff, maxtmax)
        s = p[0] * ((1 + ((t / p[1]) ** - p[2])) ** - p[3])
        return s

class SingleDagum_rch(RfuncBase):
    """Dagum distribution step function with 4 parameters A1_rch, a1_rch,
    b1_rch, p1_rch.
    This response function should be used if a two-component recharge 
    model is used and the second response function is also a SingleDagum
    response function.
    ...
    def get_init_parameters(self, name):
        ...
        parameters.loc[name + '_a_rch'] = (1, 1e-5, 5000, True, name) # 1
        parameters.loc[name + '_b_rch'] = (4, 1e-5, 5000, True, name) # 2
        parameters.loc[name + '_p_rch'] = (1, 1e-5, 5000, True, name) # 3
        return parameters
...
class DagumOsc(RfuncBase):
    """Dagum distribution step combined with an oscillating wave packet
    with 9 parameters A1, A2, a1, a2, b1, b2, p1, p2, alpha
    ...
    def get_init_parameters(self, name):
        ...
        parameters.loc[name + '_a1'] = (1.0, 1e-2, 5000, True, name) # 2
        parameters.loc[name + '_a2'] = (2.0, -5000.0, 5000.0, True, name) # 3
        parameters.loc[name + '_b1'] = (1.5, 1e-2, 5000, True, name) # 4
        parameters.loc[name + '_b2'] = (2, 1e-2, 5000.0, True, name) # 5
        parameters.loc[name + '_p1'] = (10.0, 1e-2, 5000, True, name) # 6
        parameters.loc[name + '_p2'] = (10.0, 1e-2, 5000, True, name) # 7
        parameters.loc[name + '_alpha'] = (0.9, 0.01, 0.99, True, name) # 8
        return parameters
    ...
    def step(self, p, dt=1, cutoff=None, maxtmax=None):
        t = self.get_t(p, dt, cutoff, maxtmax)
        term1 = p[0] * ((1 + ((t / p[2]) ** -p[4])) ** -p[6])
        term2 = p[1] * np.exp(-((t - p[3]) ** 2 / p[5])) * np.sin(p[7] * t - p[3])
        s = p[8] * term1 + (1 - p[8]) * term2
        return s
...
class DoubleGamma(RfuncBase):
    """Double Gamma response function with 9 parameters A1, n1, a1, b1,
    A2, n2, a2, b2 and p
    ...
    .. math:: \\theta(t) = p * A1 (t-b1)^{n1-1} e^{-(t-b1)/a1} + (1-p) * A2 (t-b2)^{n2-1} e^{-(t-b2)/a2}
    ...
    def get_init_parameters(self, name):
        parameters = DataFrame(
            columns=['initial', 'pmin', 'pmax', 'vary', 'name'])
        if self.up:
            parameters.loc[name + '_A1'] = (1 / self.meanstress, 1e-5,
                                            10, True, name)
            parameters.loc[name + '_A2'] = (1 / self.meanstress, 1e-5,
                                            10, True, name)
        elif self.up is False:
            parameters.loc[name + '_A1'] = (-1 / self.meanstress,
                                            -1e-5,
        ...
        # if n is too small, the length of response function is close to zero
        parameters.loc[name + '_n1'] = (1.0, 0.1, 100, True, name) # 2
        parameters.loc[name + '_n2'] = (1.0, 0.1, 100, True, name) # 3
        parameters.loc[name + '_a1'] = (0.1, 0.01, 1e4, True, name) # 4
        parameters.loc[name + '_a2'] = (0.1, 0.01, 1e4, True, name) # 5
        parameters.loc[name + '_b1'] = (0.0, -1e-4, 1e4, True, name) # 6
        parameters.loc[name + '_b2'] = (0.0, -1e-4, 1e4, True, name) # 7
        parameters.loc[name + '_p'] = (0.5, 0.0, 1.0, True, name) # 8
        return parameters
    ...
    def step(self, p, dt=1, cutoff=None, maxtmax=None):
        t = self.get_t(p, dt, cutoff, maxtmax)
        # s = p[0] * gammainc(p[1], t / p[2])
        term1 = p[1] * (p[8] - 1) * ((t - p[7]) ** p[3]) * (((t - p[7]) / p[5]) ** - p[3]) * gammainc(p[3], (t - p[7]) / p[5])
        term2 = p[0] * p[8] * ((t - p[6]) ** p[2]) * (((t - p[6]) / p[4]) ** - p[2]) * gammainc(p[2], (t - p[6]) / p[4])
        s = term1 - term2
        return s
