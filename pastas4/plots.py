...
from .stressmodels import RechargeModel2 as rm2
...
class Plotting:
    ...
    def results(self, tmin=None, tmax=None, figsize=(10, 8), split=False,
                adjust_height=False, tmax_step=None, **kwargs):
        ...
        tmax_step: float or list-like, optional
            Adjust the number of time steps shown in the step response plots.
            If float, all plots have the same x-range, if list-like, each of
            the plots has its own x-limit, if None, set limits automatically. 
        ...
        # check if a RechargeModel2 is present in the model, i.e., check
        # if a two component stressmodel is present
        # get all stressmodel objects
        stressmodels_objects = list(self.ml.stressmodels.items())
        
        # get all stressmodel classes
        stressmodels_types = []
        for sm in range(len(stressmodels_objects)):
            stressmodels_types.append(type(stressmodels_objects[sm][1]))

        # test if any of the present stressmodels is a RechargeModel2
        if any(sm is rm2 for sm in stressmodels_types):
            print("There is at least one Stressmodel with two stress components.")
            contribs = self.ml.get_contributions_rch(split=split, tmin=tmin,
            ...
            # number of plots (contributions and response functions) is given by
            # the number of contributions
            nplots = len(contribs)
            plot_num_sm = 0
            # create list to store step functions of recharge models with
            # two response functions
            steps_plot = []
            # get step responses
            for i, sm in enumerate(list(self.ml.stressmodels.items())):
                # check if stressmodel has two components (RechargeModel2)
                if type(sm[1]) is rm2:
                    # do everything in case there are two components

                    # get step responses
                    # steps_plot = []
                    step_quick, step_slow = self.ml.get_step_response_rch(sm[0], add_0=True)
                    steps_plot.append(step_quick)
                    steps_plot.append(step_slow)

                    # plot contributions
                    stressmodel = self.ml.stressmodels[sm[0]]
                    nsplit = stressmodel.get_nsplit()
                    if split and nsplit > 1:
                        for _ in range(nsplit):
                            ax_c1 = fig.add_subplot(gs[plot_num_sm + 2, 0], sharex=ax1)
                            ax_c2 = fig.add_subplot(gs[plot_num_sm + 3, 0], sharex=ax1)
                            contribs[plot_num_sm].plot(ax=ax_c1, x_compat=True)
                            contribs[plot_num_sm].plot(ax=ax_c2, x_compat=True)
                            ax.legend(loc=(0, 1), ncol=3, frameon=False)
                            if adjust_height:
                                ax.set_ylim(ylims[plot_num_sm+2])
                                ax.grid(True)

                    else:
                        ax_c1 = fig.add_subplot(gs[plot_num_sm + 2, 0], sharex=ax1)
                        ax_c2 = fig.add_subplot(gs[plot_num_sm + 3, 0], sharex=ax1)
                        contribs[plot_num_sm].plot(ax=ax_c1, x_compat=True, label="quick component")
                        contribs[plot_num_sm + 1].plot(ax=ax_c2, x_compat=True, label="slow component")
                        title = [stress.name for stress in stressmodel.stress]
                        if len(title) > 3:
                            title = title[:3] + ["..."]
                        ax_c1.set_title("Stress (quick): %s" %title, loc="right")
                        ax_c2.set_title("Stress (slow): %s" %title, loc="right")

                        ax_c1.legend(loc=(0, 1), ncol=3, frameon=False)
                        ax_c2.legend(loc=(0, 1), ncol=3, frameon=False)

                        if adjust_height:
                            ax_c1.set_ylim(ylims[plot_num_sm + 2])
                            ax_c2.set_ylim(ylims[plot_num_sm + 3])

                    # plot responses
                    if step_quick is not None and step_slow is not None:
                        rmax1 = max(rmax, steps_plot[plot_num_sm].index.max())
                        rmax2 = max(rmax, steps_plot[plot_num_sm+1].index.max())

                        ax_b1 = fig.add_subplot(gs[plot_num_sm + 2, 1])
                        ax_b2 = fig.add_subplot(gs[plot_num_sm + 3, 1])
                        steps_plot[plot_num_sm].plot(ax=ax_b1, label="quick rfunc")
                        steps_plot[plot_num_sm + 1].plot(ax=ax_b2, label="sloq_rfunc")

                        if tmax_step is not None:
                            if isinstance(tmax_step, float):
                                ax_b1.set_xlim(0, tmax_step)
                                ax_b2.set_xlim(0, tmax_step)
                            else:
                                ax_b1.set_xlim(0, tmax_step[plot_num_sm])
                                ax_b2.set_xlim(0, tmax_step[plot_num_sm + 1])

                    plot_num_sm += 2

                else:
                    # get step response for the case of only one component
                    step = self.ml.get_step_response(sm[0], add_0=True)
                    ...
                    if split and nsplit > 1:
                        for _ in range(nsplit):
                            ax = fig.add_subplot(gs[plot_num_sm + 2, 0], sharex=ax1)
                            contribs[plot_num_sm].plot(ax=ax, x_compat=True)
                            ax.legend(loc=(0, 1), ncol=3, frameon=False)
                            if adjust_height:
                                ax.set_ylim(ylims[plot_num_sm+2])
                                ax.grid(True)
                        ...
                        ax_c1 = fig.add_subplot(gs[plot_num_sm + 2, 0], sharex=ax1)
                        contribs[plot_num_sm].plot(ax=ax_c1, x_compat=True, label="contribution")
                        title = [stress.name for stress in stressmodel.stress]
                        if len(title) > 3:
                            title = title[:3] + ["..."]
                        ax_c1.set_title("Stress: %s" %title, loc="right")
                    ...
                    # plot responses
                    if step is not None:
                        rmax1 = max(rmax, step.index.max())

                        ax_b1 = fig.add_subplot(gs[plot_num_sm + 2, 1])
                        step.plot(ax=ax_b1, label="rfunc")
            ...
            # do everything for just one component
            print("There is no Stressmodel with two stress components.")
            # Number of rows to make the figure with
            o = self.ml.observations()
            sim = self.ml.simulate(tmin=tmin, tmax=tmax)
            res = self.ml.residuals(tmin=tmin, tmax=tmax)
    ...
    @model_tmin_tmax
    def results_rch(self, tmin=None, tmax=None, figsize=(10, 8), split=False,
        ...
        if plot_noise:
            noise = self.ml.noise(tmin=tmin, tmax=tmax)
        # contribs holds quick and slow recharge components
        contribs = self.ml.get_contributions_rch(split=split, tmin=tmin,
                                                 tmax=tmax, return_warmup=False)
        fig = plt.figure(figsize=figsize, **kwargs)
        ylims = [(min([sim.min(), o[tmin:tmax].min()]),
        ...
        if len(contribs) % len(self.ml.stressmodels) == 0:
            # int containing the number of contributions per stressmodel
            j_plot = int(len(contribs) / len(self.ml.stressmodels))

        # TODO:
        #     - implement plotting for cases where
        #         one has a number of stressmdoels
        #         with 2 contributions each and a 
        #         number of stressmodels with only
        #         one contribution
        #     - e.g., 2 SMs and 3 contributions

        # if e.g., two contributions from one
        #     stressmodel: use alternative plotting utility

        plot_num = 0

        if len(contribs) > len(self.ml.stressmodels):
            for i, sm_name in enumerate(self.ml.stressmodels):
                
                steps_plot = []
                step_quick, step_slow = self.ml.get_step_response_rch(sm_name, add_0=True)
                steps_plot.append(step_quick)
                steps_plot.append(step_slow)
                ...
                for j in range(j_plot):
                    ...
                    if split and nsplit > 1:
                        for _ in range(nsplit):
                            ax = fig.add_subplot(gs[plot_num + 2, 0], sharex=ax1)
                            contribs[plot_num].plot(ax=ax, x_compat=True)
                            ax.legend(loc=(0, 1), ncol=3, frameon=False)
                            if adjust_height:
                                ax.set_ylim(ylims[i + 2])
                                ax.grid(True)
                        ...
                        ax_c1 = fig.add_subplot(gs[plot_num + 2, 0], sharex=ax1)
                        if j == 0:
                            contribs[plot_num].plot(ax=ax_c1, x_compat=True, label="quick")
                        else:
                            contribs[plot_num].plot(ax=ax_c1, x_compat=True, label="slow")
                        title = [stress.name for stress in sm.stress]
                        if len(title) > 3:
                            title = title[:3] + ["..."]
                        
                        if j == 0:
                            ax_c1.set_title("Stresses (quick): %s" % title, loc="right")
                        else:
                            ax_c1.set_title("Stresses (slow): %s" % title, loc="right")

                        ax_c1.legend(loc=(0, 1), ncol=3, frameon=False)
                        if adjust_height:
                            ax_c1.set_ylim(ylims[i + 2])
                            ax_c1.grid(True)

                        # plot the step reponse
                        if step_quick is not None and step_slow is not None:
                            # rmax_quick = max(rmax, step_quick.index.max())
                            # rmax_slow = max(rmax, step_slow.index.max())
                            rmax = max(rmax, steps_plot[j].index.max())
                            ax_b1 = fig.add_subplot(gs[plot_num + 2, 1])
                            ...
                            steps_plot[j].plot(ax=ax_b1, label=step_label)
                ...
                #             ax.grid(True)
                # else:
                #     ax_c1 = fig.add_subplot(gs[i + 2, 0], sharex=ax1)
                #     ax_c2 = fig.add_subplot(gs[i + 3, 0], sharex=ax1)
                #     contribs[i].plot(ax=ax_c1, x_compat=True, label="quick")
                #     contribs[i + 1].plot(ax=ax_c2, x_compat=True, label="slow")
                #     title = [stress.name for stress in sm.stress]
                #     if len(title) > 3:
                #         title = title[:3] + ["..."]
                #     ax_c1.set_title("Stresses (quick): %s" % title, loc="right")
                #     ax_c2.set_title("Stresses (slow): %s" % title, loc="right")
                #     ax_c1.legend(loc=(0, 1), ncol=3, frameon=False)
                #     ax_c2.legend(loc=(0, 1), ncol=3, frameon=False)
                #     if adjust_height:
                #         ax_c1.set_ylim(ylims[i + 2])
                #         ax_c1.grid(True)
                #         ax_c2.set_ylim(ylims[i + 2])
                #         ax_c2.grid(True)
                ...
                #     rmax_quick = max(rmax, step_quick.index.max())
                #     rmax_slow = max(rmax, step_slow.index.max())
                #     ax_b1 = fig.add_subplot(gs[i + 2, 1])
                #     ax_b2 = fig.add_subplot(gs[i + 3, 1])
                #     step_quick.plot(ax=ax_b1, label="quick")
                #     step_slow.plot(ax=ax_b2, label="slow")
                #     ax_b1.legend(loc="best")
        ...
        ax1.set_xlim(tmin, tmax)
        if axb is not None:
            axb.set_xlim(0, max(rmax_quick, rmax_slow))
    ...
    def step_response(self, stressmodels=None, ax=None, figsize=None,
        ...
        # get model frequency to show correct time unit
        xaxis_label = self.ml.settings["freq"]
        plt.xlim(0)
        plt.xlabel("Time %s" %(str(xaxis_label)))
        plt.legend(legend)
        return ax
