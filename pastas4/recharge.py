...
from pastas4.decorators import njit
...
class FlexModel(RechargeBase):
    ...
    def get_init_parameters(self, name="recharge"):
        parameters = DataFrame(
            columns=["initial", "pmin", "pmax", "vary", "name"])
        parameters.loc[name + "_srmax"] = (200.0, 1e-5, 1e4, False, name)
        ...
        parameters.loc[name + "_kv"] = (1.0, 1e-6, 100.0, False, name)
        return parameters
...
class FlexModel2(RechargeBase):
    ...
    def get_init_parameters(self, name="recharge"):
        ...
        # parameters.loc[name + "_kv"] = (1.0, 0.5, 2.0, False, name)
        parameters.loc[name + "_kv"] = (1.0, 1e-6, 100.0, False, name)
        parameters.loc[name + "_omega"] = (0.1, 0.0, 1.0, False, name)
        return parameters
    ...
    def simulate(self, prec, evap, p, dt=1.0, **kwargs):
        ...
        """
        r = self.get_recharge(prec, evap, srmax=p[0], lp=p[1], ks=p[2],
                              gamma=p[3], simax=p[4], kv=p[5], omega=p[6], dt=dt)[0]
        return r

    @staticmethod
    @njit
    def get_recharge(prec, evap, srmax=250.0, lp=0.25, ks=100.0, gamma=4.0,
                     simax=2.0, kv=1.0, omega=0.1, dt=1.0):
        """
        Internal method used for the recharge calculation. If Numba is
        available, this method is significantly faster.
        ...
        ea = zeros(n, dtype=float64)  # Actual evaporation Flux
        r = zeros(n, dtype=float64)  # Recharge Flux
        r_quick = zeros(n, dtype=float64) # quick recharge flux
        si = zeros(n, dtype=float64)  # Interception Storage State
        pe = zeros(n, dtype=float64)  # Effective precipitation Flux
        ei = zeros(n, dtype=float64)  # Interception evaporation Flux
        ep = zeros(n, dtype=float64)  # Updated evaporation Flux
        ...
        for t in range(n - 1):
            ...
            # Calculate the recharge flux
            r[t] = ks * (su[t] / srmax) ** gamma + r_quick[t]
            # Calculate state of the root zone storage
            su[t + 1] = su[t] + dt * ((pe[t] * (1 - omega)) - (r[t] - r_quick[t]) - ea[t])
    ...
    def get_water_balance(self, prec, evap, p, dt=1.0, **kwargs):
        r, ea, ei, pe, sr, si = self.get_recharge(prec, evap, srmax=p[0],
                                                  lp=p[1], ks=p[2],
                                                  gamma=p[3], simax=p[4],
                                                  kv=p[5], omega=p[6], dt=dt)

        data = DataFrame(data=vstack((si, -ei, sr, pe, -ea, -r)).T,
                         columns=["Si", "Ei", "Sr", "Pe", "Ea", "R"])
        return data

class FlexModelFrac(RechargeBase):
    """Recharge to the groundwater calculate according to [collenteur_2020]_.
    Extended to support two recharge flow components: one component according
    to "standard" FlexModel and one component, which gets directly routed to
    the system from the interception storage. the parameter _omega is the
    splitting fraction where R_quick = pe * _omega and the inflow to the root
    zone storage is reduced to be pe_in = pe * (1 - _omega)
    ...
    .. math::
        ...
        \\frac{dS}{dt} = P_e - E_a - R_{slow}

    where the slow (diffuse) recharge is calculated as:
    ...
    def simulate(self, prec, evap, p, dt=1.0, **kwargs):
        ...
        """
        r, r_quick = self.get_recharge(prec, evap, srmax=p[0], lp=p[1], ks=p[2],
                                       gamma=p[3], simax=p[4], kv=p[5], omega=p[6], dt=dt)[:2]
    ...
    def get_recharge(prec, evap, srmax=250.0, lp=0.25, ks=100.0, gamma=4.0,
        ...
        su[0] = 0.5 * srmax  # Set the initial system state to half-full
        ea = zeros(n, dtype=float64)  # Actual evaporation Flux
        r_slow = zeros(n, dtype=float64)  # Recharge Flux
        r_quick = zeros(n, dtype=float64) # quick recharge flux
        si = zeros(n, dtype=float64)  # Interception Storage State
        pe = zeros(n, dtype=float64)  # Effective precipitation Flux
        ei = zeros(n, dtype=float64)  # Interception evaporation Flux
        ...
        for t in range(n - 1):
            ...
            # Calculate the recharge flux
            r_slow[t] = ks * (su[t] / srmax) ** gamma
            # Calculate state of the root zone storage
            su[t + 1] = su[t] + dt * ((pe[t] * (1 - omega)) - r_slow[t] - ea[t])

        return r_slow, r_quick, ea, ei, pe, su, si

    def get_water_balance(self, prec, evap, p, dt=1.0, **kwargs):
        r_slow, r_quick, ea, ei, pe, sr, si = self.get_recharge(prec, evap, srmax=p[0],
                                                                lp=p[1], ks=p[2],
                                                                gamma=p[3], simax=p[4],
                                                                kv=p[5], omega=p[6], dt=dt)

        data = DataFrame(data=vstack((si, -ei, sr, pe, -ea, -r_slow, -r_quick)).T,
                         columns=["Si", "Ei", "Sr", "Pe", "Ea", "R", "R_quick"])
        return data
