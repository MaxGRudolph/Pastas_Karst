...
from pastas4.decorators import PastasDeprecationWarning
from pastas4.stats.core import mean, var, std

__all__ = ["rmse", "sse", "mae", "nse", "nse_lin", "evp", "rsq", "bic", "aic",
           "pearsonr", "kge_2012"]
...
def nse_lin(obs, sim=None, res=None, missing="drop", weighted=False, max_gap=90):
    """Compute the (weighted) Nash-Sutcliffe Efficiency (NSE).
    ...
    return 1 - (abs(w * res[1:])).sum() / (abs(w * (obs[1:] - mu))).sum()
