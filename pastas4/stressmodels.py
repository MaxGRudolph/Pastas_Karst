...
from .recharge import Linear, FlexModelFrac
from .rfunc import One, Exponential, Exponential_rch, HantushWellModel
...
class StressModelBase2:
    """StressModel Base class called by the RechargeModel2 object,
    which has two recharge flow components ans two respective
    response functions.
    ...
    """
    _name = "StressModelBase2"

    def __init__(self, name, tmin, tmax, rfunc_quick=None, rfunc_slow=None):
        self.name = validate_name(name)
        self.tmin = tmin
        self.tmax = tmax
        self.freq = None
        self.rfunc_quick = rfunc_quick
        self.rfunc_slow = rfunc_slow
        self.parameters = DataFrame(
            columns=['initial', 'pmin', 'pmax', 'vary', 'name'])
    ...
    def _get_block(self, p, dt, tmin, tmax):
        ...
        if tmin is not None and tmax is not None:
            ...
            maxtmax = None
        b_quick = self.rfunc_quick.block(p[:self.rfunc_quick.nparam], dt, maxtmax=maxtmax)
        b_slow = self.rfunc_slow.block(p[self.rfunc_quick.nparam:(self.rfunc_quick.nparam + self.rfunc_slow.nparam)], dt, maxtmax=maxtmax)
        return b_quick, b_slow
...
class RechargeModel2(StressModelBase2):
    """Stressmodel simulating the effect of groundwater recharge on the
    groundwater head with two recharge flow components.
    This method was initially created to represent the duality
    of karst system recharge processes (quick and slow recharge
    component) but can be used for other purposes as well.
    ...
    evap: pandas.Series or pastas.timeseries.TimeSeries
        pandas.Series or pastas.timeseries object containing the
        evaporation series.
    rfunc_quick: pastas.rfunc class, optional
        Response function used in the convolution with the quick recharge
        component (stress). Default is Exponential. NOTE! If both the quick
        and the slow recharge component should have the same response
        function, one of them has to be given as the _rch-equivalent 
        (e.g., ps.Exponential for the quick component and ps.Exponential_rch
        for the slow component). At the moment, Exponential and SingleDagum
        rfuncs are supported for the _rch-equivalence.
    rfunc_slow: pastas.rfunc class, optional
        Response function used in the convolution with the slow recharge
        component (stress). Default is Exponential. NOTE! If both the quick
        and the slow recharge component should have the same response
        function, one of them has to be given as the _rch-equivalent 
        (e.g., ps.Exponential for the quick component and ps.Exponential_rch
    ...
    name: str, optional
        Name of the stress. Default is "recharge".
    recharge: pastas.recharge instance
        at the moment, only the FlexModelFrac recharge model is supported
        and provides two separate flow components as results. The method can
        be accessed via ps.rch.FlexModelFrac
    temp: pandas.Series or pastas.TimeSeries, optional
        pandas.Series or pastas.TimeSeries object containing the
        temperature series. It depends on the recharge model is this
        argument is required or not. At the moment, temp is not used in the
        supported context.
    cutoff: float, optional
        float between 0 and 1 to determine how long the response is (default)
        is 99.9% of the actual response time). Used to reduce computation
    ...
    -----
    This stress model computes the contribution of precipitation and
    potential evaporation in two steps. In the first step two recharge fluxes
    are computed by a model determined by the input argument `recharge`. In the
    second step these recharge fluxes are convoluted with the two response functions
    (rfunc_quick and rfunc_slow) to obtain the contribution of recharge to
    the groundwater levels.

    If a _rch-equivalent of a response function is used (e.g., ps.Exponential_rch),
    the parameters all get a trailing "_rch" (e.g., stressmodelname_A_rch) instead of the
    simple parameter name (e.g., stressmodelname_A).

    Examples
    --------
    >>> sm = ps.RechargeModel(rain, evap, rfunc_quick=ps.Exponential,
    >>>                       rfunc_slow=ps.Exponential_rch,
    >>>                       recharge=ps.rch.FlexModel(), name="rch")
    >>> ml.add_stressmodel(sm)
    ...
    """
    _name = "RechargeModel2"

    def __init__(self, prec, evap, rfunc_quick=Exponential, rfunc_slow=Exponential_rch, name="recharge",
                 recharge=FlexModelFrac(), temp=None, cutoff=0.999,
                 settings=("prec", "evap"), metadata=(None, None)):
        # Store the precipitation and evaporation time series
        self.prec = TimeSeries(prec, settings=settings[0],
                               metadata=metadata[0])
        ...
        # Calculate initial recharge estimation for initial rfunc parameters
        p = self.recharge.get_init_parameters().initial.values
        # get both recharge components from the recharge model
        meanstress = self.get_stress(p=p, tmin=index.min(), tmax=index.max(),
                                     freq=self.prec.settings["freq"])
        meanstress_quick = meanstress[0].std()
        meanstress_slow = meanstress[1].std()

        rfunc_quick = rfunc_quick(up=True, cutoff=cutoff, meanstress=meanstress_quick)
        rfunc_slow = rfunc_slow(up=True, cutoff=cutoff, meanstress=meanstress_slow)

        StressModelBase2.__init__(self, name=name, tmin=index.min(),
                                 tmax=index.max(), rfunc_quick=rfunc_quick,
                                 rfunc_slow=rfunc_slow)
    ...
    def set_init_parameters(self):
        """Internal method to set the initial parameters."""
        self.parameters = concat(
            [self.rfunc_quick.get_init_parameters(self.name),
             self.rfunc_slow.get_init_parameters(self.name),
             self.recharge.get_init_parameters(self.name)
             ])
    ...
    def simulate(self, p=None, tmin=None, tmax=None, freq=None, dt=1.0,
        ...
        if p is None:
            p = self.parameters.initial.values
        b_quick, b_slow = self._get_block(p[:-self.recharge.nparam],
                                          dt, tmin, tmax)
        # both stresses (quick and slow recharge) are in stress
        stress = self.get_stress(p=p, tmin=tmin, tmax=tmax, freq=freq,
                                 istress=istress)

        # quick component
        stress_quick = stress[0].values
        # slow component
        stress_slow = stress[1].values
        ...
        # do convolution of both stresses
        component_quick = Series(data=fftconvolve(stress_quick, b_quick, "full")[:stress[0].size],
                                 index=self.prec.series.index, name=name, fastpath=True)
        component_slow = Series(data=fftconvolve(stress_slow, b_slow, "full")[:stress[1].size],
                                 index=self.prec.series.index, name=name, fastpath=True)
        # component_add = component_quick.add(component_slow)

        # return component_add

        # return quick and slow component separately
        return component_quick, component_slow

    def get_stress(self, p=None, tmin=None, tmax=None, freq=None,
                   istress=None, **kwargs):
        """Method to obtain the recharge stresses calculated by the two
        component recharge model.
        ...
        -------
        stress: pandas.Series
            When no istress is selected, this return the two estimated recharge
            fluxes that are convoluted with two respective functions on the
            "simulate" method.
        ...
        if istress is None:
            ...
            # in recharge.simulate, slow component is returned first
            stress_slow, stress_quick = self.recharge.simulate(prec=prec, evap=evap, temp=temp,
                                            p=p[-self.recharge.nparam:])

            # return quick recharge component first, slow recharge component second
            return Series(data=stress_quick, index=self.prec.series.index, name="rch_quick", fastpath=True), Series(data=stress_slow, index=self.prec.series.index, name="rch_slow", fastpath=True)
    ...
    def to_dict(self, series=True):
        data = {
            ...
            "prec": self.prec.to_dict(),
            "evap": self.evap.to_dict(),
            "rfunc_quick": self.rfunc_quick._name,
            "rfunc_slow": self.rfunc_slow._name,
            "name": self.name,
            "recharge": self.recharge._name,
            "cutoff": self.rfunc.cutoff,
            "temp": self.temp.to_dict() if self.temp else None
