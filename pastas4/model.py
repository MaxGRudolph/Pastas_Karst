...
class Model:
    ...
    def simulate(self, p=None, tmin=None, tmax=None, freq=None, warmup=None,
                 return_warmup=False, return_contributions=False):
        ...
        for sm in self.stressmodels.values():
            ...
            # add two contributions if RechargeModel2 is used
            #     (applies especially for the combination of
            #     RechargeModel2 + FlexModelFrac)
            if len(contrib) == 2:
                sim = sim.add(contrib[0])
                sim = sim.add(contrib[1])
                if return_contributions:
    ...
    @get_stressmodel
    def get_contribution_rch(self, name, tmin=None, tmax=None, freq=None,
                         warmup=None, istress=None, return_warmup=False,
                         p=None):
        """Method to get the contribution of a two-component
        recharge model (e.g., FlexModelFrac). This also works
        if also a standard one component recharge model is present.
        ...
        if len(contrib) == 2:
            contrib_quick = contrib[0]
            contrib_slow = contrib[1]

            # Respect provided tmin/tmax at this point, since warmup matters for
            # simulation but should not be returned, unless return_warmup=True.
            if not return_warmup:
                contrib_quick = contrib_quick.loc[tmin:tmax]
                contrib_slow = contrib_slow.loc[tmin:tmax]

            return contrib_quick, contrib_slow
    ...
    def get_contributions_rch(self, split=True, **kwargs):
        """Method to get contributions of all two component recharge
        models (e.g., FlexModelFrac).
        ...
        for name in self.stressmodels:
            ...
            if split and nsplit > 1:
                for istress in range(nsplit):
                    contrib = self.get_contribution_rch(name, istress=istress, **kwargs)
                    # append contributions
                    # even if a two component recharge model was used, a one component
                    # recharge model could be used in parallel as well so the number of
                    # contributions in contrib has to be checked
                    if len(contrib) == 2:
                        contribs.append(contrib[0])
                        contribs.append(contrib[1])
                    else:
                    ...
                    contrib = self.get_contribution_rch(name, **kwargs)
                    # append contributions
                    # even if a two component recharge model was used, a one component
                    # recharge model could be used in parallel as well so the number of
                    # contributions in contrib has to be checked
    ...
    def _get_response_rch(self, block_or_step, name, p=None, dt=None, add_0=False,
                          **kwargs):
        """Internal method to compute the block and step response for a two component
        recharge model (e.g., FlexModelFrac).
        ...
        Returns
        -------
        response_quick: pandas.Series of the quick response
        response_slow: pandas.Series of the slow response

        """
        if self.stressmodels[name].rfunc_quick is None:
            self.logger.warning("Stressmodel {} has no rfunc_quick".format(name))
            return None
        elif self.stressmodels[name].rfunc_slow is None:
            self.logger.warning("Stressmodel {} has no rfunc_slow".format(name))
            return None
        else:
            block_or_step_quick = getattr(self.stressmodels[name].rfunc_quick,
                                          block_or_step)
            block_or_step_slow = getattr(self.stressmodels[name].rfunc_slow,
                                         block_or_step)
        ...
        response_quick = block_or_step_quick(p[:self.stressmodels[name].rfunc_quick.nparam], dt, **kwargs)
        response_slow = block_or_step_slow(p[self.stressmodels[name].rfunc_quick.nparam:(self.stressmodels[name].rfunc_quick.nparam + self.stressmodels[name].rfunc_slow.nparam)], dt, **kwargs)


        if add_0:
            response_quick = np.insert(response_quick, 0, 0.0)
            response_slow = np.insert(response_slow, 0, 0.0)

        if isinstance(dt, np.ndarray):
            t_quick, t_slow = dt
        else:
            t_quick = np.linspace(dt, response_quick.size * dt, response_quick.size)
            t_slow = np.linspace(dt, response_slow.size * dt, response_slow.size)
        response_quick = Series(response_quick, index=t_quick, name=name + "_quick")
        response_slow = Series(response_slow, index=t_slow, name=name + "_slow")
        response_quick.index.name = "Time [days]"
        response_slow.index.name = "Time [days]"

        return response_quick, response_slow
    ...
    @get_stressmodel
    def get_block_response_rch(self, name, p=None, add_0=False, dt=None, **kwargs):
        """Method to obtain the block responses for a stressmodel with two flow
        components such as the recharge model FlexModelFrac.
        ...
        Returns
        -------
        b_quick: pandas.Series
            Pandas.Series with the block response of the quick component. The
            index is based on the frequency that is present in the model.settings.
        b_slow: pandas.Series
            Pandas.Series with the block response of the slow component. The
            index is based on the frequency that is present in the model.settings.

        """
        return self._get_response_rch(block_or_step="block", name=name, dt=dt,
                                      p=p, add_0=add_0, **kwargs)
    ...
    @get_stressmodel
    def get_step_response_rch(self, name, p=None, add_0=False, dt=None, **kwargs):
        """Method to obtain the step responses for a stressmodel with two flow
        components such as the recharge model FlexModelFrac.
        ...
        Returns
        -------
        s_quick: pandas.Series
            Pandas.Series with the step response of the quick component. The
            index is based on the frequency that is present in the model.settings.
        s_slow: pandas.Series
            Pandas.Series with the step response of the slow component. The
            index is based on the frequency that is present in the model.settings.
        """
        return self._get_response_rch(block_or_step="step", name=name, dt=dt,
                                      p=p, add_0=add_0, **kwargs)
    ...
    def get_stress(self, name, tmin=None, tmax=None, freq=None, warmup=None,
        ...
        if not return_warmup:
            # check case of RechargeModel2
            #     (stress is tuple / list with two stresses)
            if len(stress) == 2:
                stress_quick = stress[0].loc[tmin:tmax]
                stress_slow = stress[1].loc[tmin:tmax]
                stress = [stress_quick, stress_slow]
            else:
    ...
    def fit_report(self, output="full"):
        ...
        parameters = self.parameters.loc[:, ["optimal", "stderr",
                                             "initial", "vary"]]
        stderr = (parameters.loc[:, "stderr"] / parameters.loc[:, "optimal"]) * 100.0
        parameters.loc[:, "stderr"] = stderr.abs().apply("\u00B1{:.2e}%".format)
